        	    ...
        	    )
{
  ...
  conf->keys.button_A= SDLK_x;
  conf->keys.button_B= SDLK_z;
  conf->keys.select= SDLK_RETURN;
           ...
           )
{
  ...
  dst->keys= src->keys;
  dst->screen_size= src->screen_size;
  dst->scaler= src->scaler;
  dst->bios_fn= src->bios_fn!=NULL ? g_strdup ( src->bios_fn ) : NULL;
           ...
           )
{
  ...
  get_key_value ( f, "key-button-A", &(conf->keys.button_A) );
  get_key_value ( f, "key-button-B", &(conf->keys.button_B) );
  get_key_value ( f, "key-select", &(conf->keys.select) );
        	   ...
        	   )
{
  ...
  set_key_value ( f, "key-button-A", conf->keys.button_A );
  set_key_value ( f, "key-button-B", conf->keys.button_B );
  set_key_value ( f, "key-select", conf->keys.select );
          ...
          )
{
  ...
  else                        copy_conf ( conf, default_conf );
