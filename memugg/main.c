       ...
       )
{
  ...
  err= NULL;
  context= g_option_context_new ( "<rom> - executa ROMS de Game Gear" );
              ...
              )
{
  ...
  switch ( header->region )
    {
    case GG_ROM_SMS_JAPAN: printf ( "SMS Japó\n" ); break;
    case GG_ROM_SMS_EXPORT: printf ( "SMS Exportació\n" ); break;
    case GG_ROM_GG_JAPAN: printf ( "GG Japó\n" ); break;
    case GG_ROM_GG_EXPORT: printf ( "GG Exportació\n" ); break;
    case GG_ROM_GG_INTERNATIONAL: printf ( "GG Internacional\n" ); break;
    case GG_ROM_UNK:
    default: printf ( "Desconeguda\n" );
    }
  printf ( "Grandària: " );
  if ( header->size == -1 ) printf ( "--\n" );
  else                     printf ( "%d KB\n", header->size );
  printf ( "Checksum:  %04x\n", header->checksum );
  ...
  if ( init_lock ( rom_id, opts->verbose ) )
    {
      ...
      // Executa.
      frontend_run ( &rom, rom_id, opts->sram_fn,
