...
error_dialog_draw (void)
{
  ...
  static const int fgcolor= 0x0FF;
  static const int bgcolor= 0x00F;
           ...
           )
{
  ...
  frontend_reconfigure ( &conf, "memuGG ");
  */
  ...
  frontend_reconfigure ( _conf, "memuGG" );
  */
  GG_rom_free ( rom );
...
init_fchooser (void)
{
  ...
  _fchooser.fsel= filesel_new ( cdir, "[.]gg$" );
  ...
  _fchooser.fgcolor_cdir= 0x00F; /* Roig. */
  _fchooser.fgcolor_entry= 0x000; /* Negre. */
  _fchooser.fgcolor_selected_entry= 0x00F; /* Roig. */
  _fchooser.fgcolor_sc= 0x00F; /* Roig. */
  ...
  _fchooser.mouse_color_white= 0xBBB;
...
fchooser_draw (void)
{
  ...
  for ( n= _fchooser.first; n <= _fchooser.last; ++n )
    {
      node= &(_fchooser.v[n]);
      effect_interpolate_color ( _fb, 8, offy, WIDTH-16,
                                 ENTRY_HEIGHT, 0xFFF, 0.60 );
