...
/* El nom del fitxer a gastar per a la SRAM, si és NULL es gasta el
   per defecte. */
...
close_sram (void)
{
  ...
  if ( fd == NULL )
    error ( "no s'ha pogut crear el fitxer temporal '%s': %s",
            tmp, strerror ( errno ) );
  if ( fwrite ( _mem, SRAM_SIZE, 1, fd ) != 1 )
    {
      fclose ( fd );
      remove ( tmp );
  ...
  if ( g_file_test ( name, G_FILE_TEST_IS_REGULAR ) )
    {
      ...
      if ( fread ( _mem, SRAM_SIZE, 1, f ) != 1 )
        error ( "no s'ha pogut llegir la SRAM de '%s': %s",
        	name, strerror ( errno ) );
      fclose ( f );
    }
  else memset ( _mem, 0, SRAM_SIZE );
  if ( name != _sram_fn ) g_free ( (void *) name );
