...
send_signal (void)
{
  ...
  // Crea la senyal
  signal= dbus_message_new_signal ( "/memu/NES/object",
                                    (const char *) _ifname,
                                    "ShowWin" );
  if ( signal == NULL )
    {
          ...
          )
{
  ...
  buf= g_string_new ( "memus.NES" );
  if ( sname != NULL )
    g_string_append_printf ( buf, ".%s", sname );
  if ( !is_base )
    {
      if ( romid != NULL )
        g_string_append_printf ( buf, ".rom%s", romid );
      else
           ...
           )
{
  ...
  // Crea el nom base.
  name= get_name ( TRUE, NULL );
  ret= create_name ( name, romid==NULL, verbose, &err );
  g_free ( name );
  
  // Crea el subnom
  if ( romid != NULL )
    {
      // En aquest no importa que hi haguera una altra aplicació
      // sempre i quan no estiga en marxa norom.
      // 1.- Intentem agafar el nom de norom
      name= get_name ( FALSE, NULL );
      ret= create_name ( name, false, verbose, &err );
      // 2.- Si ho aconseguim agafem el que toca
      if ( ret )
        {
          name2= get_name ( FALSE, romid );
          ret= create_name ( name2, true, verbose, &err );
          g_free ( name2 );
        }
      // 3.- Allibera norom en quasevol cas
      if ( dbus_bus_release_name ( _con, name, &err ) == -1 )
        {
          if ( dbus_error_is_set ( &err ) )
            error ( "error a l'alliberar %s: %s - %s",
                    name, err.name, err.message );
          else
            error ( "error a l'alliberar %s", name );
        }
      if ( verbose )
        fprintf ( stderr, "Alliberat nom D-BUS: %s\n", name );
      g_free ( name );
    }
