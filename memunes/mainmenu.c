        	...
        	)
{
  ...
  /* Fons. */
  effect_fade ( _fb, x, y, width, height, -1 );
...
error_dialog_draw (void)
{
  ...
  static const int fgcolor= 0x28;
  static const int bgcolor= 0x16;


  draw_rectangle ( x*8-1, y*8-1, width*8+2, height*8+2, 0x000 );
  draw_rectangle ( x*8, y*8, width*8, height*8, bgcolor );
  tiles8b_draw_string ( _fb, BG_WIDTH, "ERROR!!", x+1, y+1,
        		fgcolor, bgcolor, 0 );
...
init_fchooser (void)
{
  ...
  _fchooser.fsel= filesel_new ( cdir, "[.]nes$" );
  ...
  _fchooser.fgcolor_cdir= 0x16; /* Roig. */
  _fchooser.fgcolor_entry= 0x0F; /* Negre. */
  _fchooser.fgcolor_selected_entry= 0x16; /* Roig. */
  _fchooser.fgcolor_sc= 0x16; /* Roig. */
  ...
  _fchooser.mouse_color_black= 0x0F;
  _fchooser.mouse_color_white= 0x20;
...
fchooser_draw (void)
{
  ...
  // Directori actual.
  effect_fade ( _fb, 8, OFFY, BG_WIDTH-16, 10, -2 );
  ...
  for ( n= _fchooser.first; n <= _fchooser.last; ++n )
    {
      node= &(_fchooser.v[n]);
      effect_fade ( _fb, 8, offy, BG_WIDTH-16, ENTRY_HEIGHT, 3 );
